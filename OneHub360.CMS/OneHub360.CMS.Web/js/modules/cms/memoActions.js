OneHub360.controller('memoActions',
    ['$scope', '$http', '$timeout', 'ngToast', '$uibModal','$state', function
        ...
        {
            $scope.memoId = Id;
            $scope.downloadUrl = cmsContext.cmsServiceBaseUrl + 'cms/draftmemo/download/' + $scope.memoId;
        }

        $scope.PrintAppLink = '/components/modules/cms/webprint/OneHub360.CMS.Web.Print.xbap?Id=' + $state.memoId + '&callBackFunction=ReloadAfterScan';
        ...
        $scope.InitForOutgoing = function (Id) {
            $scope.memoId = Id;
            $scope.downloadUrl = cmsContext.cmsServiceBaseUrl + 'cms/outgoingmemo/download/' + $scope.memoId;
        }

        $scope.InitForIncoming = function (Id) {
            $scope.memoId = Id;
            $scope.downloadUrl = cmsContext.cmsServiceBaseUrl + 'cms/incomingmemo/download/' + $scope.memoId;
        }
        ...
        $scope.sendDraftMemo = function()
        {
            ...
            var webApiCall = cmsContext.cmsServiceBaseUrl + 'cms/draftmemo/send/' + $scope.memoId + '/' + HubStorage().currentUserId;

            $timeout(function () {
                $http.get(webApiCall)
                   .success(function (data, status, headers, config) {
                       $scope.sendTitle = "إرسال";
                       ngToast.create('تم إرسال المذكرة بنجاح');
                       $scope.$emit('reloadAfterAction', $scope.memoId);
                       $scope.$emit('removeFeedItemFromView', $scope.memoId);
                       $scope.actionStatus = 1;
                   })
        ...
        {
            var webApiCall = cmsContext.cmsServiceBaseUrl + 'cms/draftmemo/print/' + $scope.memoId;
            $timeout(function () {
                $http.get(webApiCall)
                   .success(function (data, status, headers, config) {
        ...
        $scope.ShowForwardIncomingMemo = function (Id,Subject,ThreadId)
        {
            
            $scope.action = new Object();
            $scope.action.Subject = Subject;
            $scope.action.FK_IncomingMemoId = Id;
            ...
            $scope.action.Title = 'تحويل مراسلة واردة';
        ...
        $scope.GetIncomingMemoAttachements = function () {
            var webApiCall = cmsContext.cmsServiceBaseUrl + 'cms/incomingmemo/documents/' + $scope.memoId;
        ...
        $scope.ForwardIncomingMemo = function (send) {
            ...
            var webApiCall = cmsContext.cmsServiceBaseUrl + 'cms/incomingmemo/send';
