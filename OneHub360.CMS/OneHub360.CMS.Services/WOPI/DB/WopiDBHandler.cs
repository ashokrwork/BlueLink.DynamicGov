...
namespace OneHub360.CMS.Services.WOPI.DB
{
    /// <summary>
    ...
    /// </remarks>
    public class WopiDBHandler : IHttpHandler
    {
        private const string WopiPath = @"/wopidb/";
        private const string WordRequestPath = @"word/";
        ...
        private static WopiRequest ParseRequest(HttpRequest request)
        {
            ...
            if (wopiPath.StartsWith(FilesRequestPath))
            {
                ...
                if (rawId.EndsWith(ContentsRequestPath))
                {
                    ...
                    requestData.Id = Path.GetFileNameWithoutExtension(rawId.Substring(0, rawId.Length - ContentsRequestPath.Length));
                ...
                {
                    requestData.Id = Path.GetFileNameWithoutExtension(rawId);
        ...
        private void HandleCheckFileInfoRequest(HttpContext context, WopiRequest requestData)
        {
            ...
            try
            {
                var checkFileInfo = new CheckFileInfoResponse();

                using (var documentWorker = new DocumentWorker(WorkerMode.NonQueue))
                {

                    checkFileInfo = documentWorker.GetFileInfo(Guid.Parse(requestData.Id));
                }

                Guid temp;
                if (Guid.TryParse(requestData.UserId, out temp))
                {

                    var feedApiClient = new FeedApiClient();
                    var userInfo = feedApiClient.GetUser(requestData.UserId).Result;
                    checkFileInfo.UserFriendlyName = userInfo.FullName;
                    checkFileInfo.OwnerId = userInfo.Id.ToString();
                    checkFileInfo.HostAuthenticationId = userInfo.Id.ToString();
                }
                string jsonString = JsonConvert.SerializeObject(checkFileInfo);
        ...
        private void HandleGetFileRequest(HttpContext context, WopiRequest requestData)
        {
            ...
            try
            {
                ...
                using (var documentWorker = new DocumentWorker(WorkerMode.NonQueue))
                {
                    fileContent = documentWorker.GetFileContent(Guid.Parse(requestData.Id));
                }
        ...
        private void HandlePutFileRequest(HttpContext context, WopiRequest requestData)
        {
            ...
            lock (Locks)
            {
                hasExistingLock = TryGetLock(GetLockKey(requestData), out existingLock);
            }
            ...
            //if (!hasExistingLock && document.FileBinary.Length != 0)
            //{
            ...
            try
            {
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    //using (var file = WordprocessingDocument.Open(memoryStream, false))
                    //{
                    //     int.TryParse(file.ExtendedFilePropertiesPart.Properties.Pages.Text,out pagesCount);
                    //}

                    using (var documentWorker = new DocumentWorker(WorkerMode.NonQueue))
                    {
                        documentWorker.SaveFileContent(Guid.Parse(requestData.Id), memoryStream.ToArray());
                    }
                }
            }
            catch (UnauthorizedAccessException)
        ...
        private void HandleLockRequest(HttpContext context, WopiRequest requestData)
        {
            ...
            {
                LockInfo existingLock;
                if (TryGetLock(GetLockKey(requestData), out existingLock))
                {
                ...
                else
                {
                    ...
                    Locks[GetLockKey(requestData)] = new LockInfo() { DateCreated = DateTime.UtcNow, Lock = newLock };
        ...
        private string GetLockKey(WopiRequest requestData)
        {
            return (requestData.Id + "-" +requestData.UserId);
        }
            ...
            lock (Locks)
            {
                ...
                if (TryGetLock(GetLockKey(requestData), out existingLock))
                {
                    if (existingLock.Lock == newLock)
                    {
                        ...
                        Locks.Remove(GetLockKey(requestData));
                        ReturnSuccess(context.Response);
                    }
                    else
                    {
